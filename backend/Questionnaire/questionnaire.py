...
def get_all_questions_dto():
    result = []
    for question in questions:
        result.append(QuestionDTO(question=question.get_question(), answers=question.get_answers()))
    return result

def get_workout_recommendation(answers: List[str]):
    total_score = 0
    for i, user_answer in enumerate(answers):
        if i < len(questions):
            total_score += questions[i].get_score_of_response(user_answer)
            
    return get_workout(total_score)